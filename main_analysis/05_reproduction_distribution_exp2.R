...
dat <- read_csv('data/processed/all_data_filtered_exp2.csv', col_types = cols())
...
rep_proportion_zero_bf <- 
  rep_proportion_zero_bf %>% 
  mutate(hjust=case_when(contrast_level == 'absent' ~ 'left',
                         contrast_level == 'low' ~ 'left',
                         contrast_level == 'high' ~ 'right'))

# Coordinates for plotting BF and Cohen's d
x_prop_zero <- c(c(1.3, 1.7, 3.5),
                 c(1.3, 1.7, 3.5),
                 c(1.3, 1.7 , 3.5))
y_prop_zero <- c(c(1, .82, 0.05),
                 c(1, .76, 0.05),
                 c(1, .72, 0.05))
...
p_proportion_zero <-
  ...
  geom_text(data=rep_proportion_zero_bf, 
            ...
            size=2.7, color='black', parse = TRUE, inherit.aes = FALSE) +
  geom_text(data=rep_proportion_zero_bf, 
...
p_rep_non_zero <- 
  ...
  geom_errorbarh(data=rep_all_data_summary %>% filter(bias_direction=='present'), 
                 aes(y=-50, xmin=ymin, xmax=ymax, x=y),
                 height=50, show.legend = FALSE) +
  geom_errorbarh(data=rep_all_data_summary %>% filter(bias_direction=='control'), 
                 aes(y=-35, xmin=ymin, xmax=ymax, x=y),
                 height=50, show.legend = FALSE) +
  # add mean
  ...
  scale_x_continuous(breaks = c(0.00, 0.03, 0.06), limits = c(0, .085))

# coordinates to plot BF and Cohen's d value
x_dist=rep(c(.05,.05,.05), 3)
y_dist=rep(c(480, 480, 480), 3)
