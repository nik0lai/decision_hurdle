...
x_prop_zero <- c(c(1.2, 2, 3.2),
                 c(1.2, 2, 3.2),
                 c(1.2, 2 , 3.2))
y_prop_zero <- c(c(1.02, .74, -.05),
                 c(1.02, .6, -.05),
                 c(1.02, .62, -.05))
...
x_dist=rep(c(.095,.095,.095), 3)
y_dist=rep(c(460, 460, 460), 3)
...
((p_proportion_zero) / (p_rep_non_zero)) 
  # plot_layout(heights = c(.49, .005, .49)) 
  # theme(legend.position = 'top') &
  # guides(color='none',
  #        shape='none',
  #        fill=guide_legend('Bias direction', override.aes = list(shape=c(NA,NA), fill=c('gray', 'gray10'), color=c('gray', 'gray10'))))

ggsave('plots/fig_exp1Rep.png', width = 6, height = 8, scale=.9, dpi=600, device = png, bg='white')
